...
The following code locations are impacted:
{found_edit_locs}
...
def gen_test(instance_id, args, swe_bench_data, prev_o, extra_info_lookup, write_lock=None):
    ...
    for file, locs in extra_info.get("found_related_locs", {}).items():
        for loc in locs:
            # split locs by newline if needed
            found_related_locs.extend(
                f"{file}: {single_loc.strip()}"
                for single_loc in loc.split("\n")
                if single_loc.strip()
            )

    # Extract found_edit_locs (robust against both dict and list)
    found_edit_locs_raw = extra_info.get("found_edit_locs", {})

    # Normalize: if it's a dict, wrap it in a list
    if isinstance(found_edit_locs_raw, dict):
        found_edit_locs_raw = [found_edit_locs_raw]

    found_edit_locs = []
    for edit_entry in found_edit_locs_raw:
        for file, locations in edit_entry.items():
            for loc in locations:
                if loc.strip():
                    found_edit_locs.append(f"{file}: {loc}")
    ...
    message = prompt_template.format(problem_statement=problem_statement,
            found_files="\n".join(found_files),
            found_related_locs="\n".join(found_related_locs),
            found_edit_locs="\n".join(found_edit_locs),
    ).strip()
...
def generate_tests(args):
    ...
    extra_info_data = load_jsonl(args.extra_info_file)
    extra_info_lookup = {entry["instance_id"]: entry for entry in extra_info_data}
    ...
    if args.num_threads == 1:
        for instance_id in tqdm(instances, total=len(instances), colour="MAGENTA"):
            gen_test(instance_id, args, swe_bench_data, prev_o, extra_info_lookup)
    else:
        write_lock = Lock()
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(
                    gen_test, instance_id, args, swe_bench_data, prev_o, extra_info_lookup, write_lock,
                ): instance_id
...
def main():
    ...
    parser.add_argument(
                        ...
                        type=str,
                        required=True,
                        help="Path to the JSONL file with extra info like found_files, found_related_locs, found_edit_locs",
    )
    parser.add_argument(
        "--select_id",
        type=int,
