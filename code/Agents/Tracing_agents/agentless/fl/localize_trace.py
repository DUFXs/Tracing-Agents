...
def localize_irrelevant_instance(
    ...
    if args.target_id is not None and args.target_id != bug["instance_id"]:
        print("SKIPPED IN LOCALIZE IRRELEVANT INSTANCE!!")
        return
...
def localize_instance(
    ...
    if args.target_id is not None and args.target_id != bug["instance_id"]:
        print("SKIPPED IN LOCALIZE INSTANCE!!")
        return
    ...
    print("GETTING STRUCTURE......", instance_id, args.target_id)
    structure = get_repo_structure(
        instance_id, bug["repo"], bug["base_commit"], "playground"
    )
...
def localize_irrelevant(args):
    ...
    if args.num_threads == 1:
        for bug in tqdm(swe_bench_data, colour="MAGENTA"):
            if args.target_id is None or bug["instance_id"] == args.target_id:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            #         write_lock,
            #     )
            #     for bug in swe_bench_data if args.target_id is None or bug["instance_id"] == args.target_id #Ira modified
            # ]
            futures = []
            ...
            for future in tqdm(
                ...
                total=len(futures), #total=len(swe_bench_data), 
                colour="MAGENTA",
            ):
                future.result()
...
def localize(args):
    ...
    if args.target_id is not None:
        print("Got here...........")
        # swe_bench_data = [bug for bug in swe_bench_data if bug["instance_id"] == args.target_id] #Ira modified
    ...
    if args.num_threads == 1:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in tqdm(
                concurrent.futures.as_completed(futures),
                total=len(futures), #total=len(swe_bench_data) # Ira modified
                colour="MAGENTA",
            ):
                future.result()
